...
### V2 Phase 0c — Admin-provisioned account activation — BUILT and CLOSED [2026-07-23]

**Superseded twice in one thread**: originally planned as a plain self-signup form (name/email/password, no admin step) after Phase 0's RLS decision made the Zoho-user dropdown pointless. The owner then specified a different, more deliberate flow the same day this actually got built: admin pre-adds a person (name+email, no password) via the SQL Editor; the person activates their own account by picking their name, setting a password, and verifying a one-time emailed code; only then can they sign in with email+password like normal. This is what shipped — the plain-3-field version below was never built.

**What shipped**:
- `supabase/migrations/008_pending_users.sql` — new `pending_users` table (full_name, email, no password), RLS enabled with **no policies** (service-role only, same pattern as `sync_state`). Deliberately outside the `auth` schema — never hand-insert into `auth.users` directly (unsupported/fragile); real accounts are created via `supabase.auth.admin.createUser()`.
- `app/api/account/pending/route.ts` (GET, unauthenticated) — lists `{id, full_name}` for the name picker, service-role client (RLS has no policies, so anon key would see nothing).
- `app/api/account/activate/route.ts` (POST `{pendingUserId, password}`) — looks up the trusted email/name by ID (never trusts a client-supplied email), calls `admin.createUser({email, password, email_confirm:false})` (idempotent-safe: an "already registered" error from a retried attempt is swallowed, not failed), then `signInWithOtp({email, shouldCreateUser:false})` to send the one-time code. Returns `{email}` for the client to display.
- `app/api/account/verify/route.ts` (POST `{email, code}`) — `verifyOtp({email, token:code, type:"email"})` via the cookie-aware server client (session lands directly in the response cookie — no separate client-side sign-in call needed), then deletes the now-used `pending_users` row.
- `proxy.ts` — `/api/account/*` added to the public (pre-auth) path list, same reasoning as `/login`.
- `app/login/page.tsx` — rewritten as 3 modes: `signin` (unchanged email+password), `setup-details` (pick name from `/api/account/pending`, set password → calls activate), `setup-code` (enter the emailed code → calls verify → redirects in). Also added the indeterminate CSS spinner this phase always wanted (no new dependency) — folded in since it touched the same submit buttons anyway.
- `app/api/zoho/users/route.ts` is now fully unused (superseded, not just by the plain-3-field plan but by this too) — still flagged as a cleanup candidate, not deleted.

**Verified live end-to-end** (dev server, real Supabase — not mocked): seeded a `pending_users` row → `/api/account/pending` listed it → `/api/account/activate` created the `auth.users` row and sent the OTP → retrieved the real code via `admin.generateLink()`'s `properties.email_otp` (no inbox access needed for testing) → `/api/account/verify` returned `200` with a valid session cookie, confirmed `email_confirmed_at` set, `pending_users` row deleted, `public.users` row created with the right name. Also confirmed: a wrong code returns a clean `400` (not a crash), and the password set during activation immediately works for a normal `signInWithPassword` afterward — OTP is genuinely first-time-setup-only, never required again, per the owner's explicit confirmation.

**Exit check — all passed**: admin can add a person without touching code; that person activates via name-pick + password + one code, never touching Zoho or typing their own email; subsequent logins are plain email+password; wrong-code and retry-after-partial-setup are both handled without crashing.
